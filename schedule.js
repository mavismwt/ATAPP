...
function getStatus(aData) {
	if (aData["type"] == 1) {
		...
		let all = 0;
		let present = 0;
		for (var modelId in aData["model"]) {
			if (aData["time"]["now"] >= modelId) {
				present += aData["model"][modelId]["weight"];
			}
			all += aData["model"][modelId]["weight"];
		}
		let toEnd = modelId - aData["time"]["now"];
		let status = parseInt(present*100/all);
		let statusCheck = status > 100 ?  100 : status;
...
function addSchedule(newSchedule) {
	...
	createJson(id, newSchedule, _data); //将新数据添加到json对象中
...
function changeSchedule(id, schedule) {
	...
	createJson(id,schedule,_data); //在json对象中修改数据
...
function deleteSchedule(id) {
	...
	createJson(id,undefined,_data); //从json对象中删除数据
